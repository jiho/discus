...
Written by Jean-Olivier Irisson.
 */

//public class Auto_Tracking implements PlugInFilter, MouseListener, Measurements  {
public class Stack_Corrector implements PlugInFilter, Measurements {
  ...
  public int setup(String arg, ImagePlus img) {
	...
	public void run(ImageProcessor ip) {
		...
		if (nFrames<2) { // if number of frames <= 1 we don't have a stack
			IJ.showMessage("Stack Corrector", "Stack required");
			return;
		}
      ...
      GenericDialog gderr2 = new GenericDialog("Auto Tracking");  
		...
		float[] x_coords = new float [nFrames][2];
		float[] y_coords = new float [nFrames][2];// in coords we store the coordinates and the distance
		...
		// Write the results to the coordinates tables
			x_coords[i][1] = xDest[1];
			y_coords[i][1] = yDest[1];
